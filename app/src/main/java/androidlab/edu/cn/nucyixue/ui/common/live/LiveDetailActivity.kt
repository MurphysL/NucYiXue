...
class LiveDetailActivity : AppCompatActivity(){
    ...
    private fun updateInfo(){
        live?.let {
            val lu = LU()
            lu.liveId = it.objectId
            lu.userId = AVUser.getCurrentUser().objectId
            lu.saveInBackground(object : SaveCallback(){
                override fun done(p0: AVException?) {
                    if(p0 == null){
                        if(live!!.isText == LCConfig.LIVE_TEXT) enterLive(live!!) else enterVideo(live!!)
                    }else{
                        toast("Unknown Error")
    ...
    private fun pay(t : Live){
        ...
        BP.pay(t.objectId, t.name, t.price.toDouble(), true, object : PListener {
            ...
            override fun fail(p0: Int, p1: String?) {
                if (p0 == -3) {
                    toast("监测到你尚未安装支付插件,无法进行支付,请先安装插件(已打包在本地,无流量消耗),安装结束后重新支付")
                } else {
                    toast("支付中断!")
                }
                hideDialog()
                        ...
                        conversation.addMembers(member, object : AVIMConversationCallback(){
                            override fun done(p0: AVIMException?) {
                                if(p0 == null){
                                    ...
                                    val intent = Intent(this@LiveDetailActivity, ConversationActivity::class.java)
