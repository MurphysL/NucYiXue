package androidlab.edu.cn.nucyixue.ui.common.live
...
class CreateLiveActivity : AppCompatActivity(){
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQUEST_CODE_CHOOSE && resultCode == RESULT_OK) {
            ...
            select_uri?.let {
                ...
                val bitmap = BitmapFactory.decodeFile(FileUtils.getFilePathFromUri(this, it))
                ...
                Service.api_keyword.getKeyword(live_summary, 4)
                        ...
                        .subscribe {
                            ...
                            if (keyword.isNotEmpty()) {
                                ...
                                val path = FileUtils.getFilePathFromUri(this, select_uri!!)
                                path?.let {
    ...
    private fun createLiveWithPic(live_name : String, live_time : Date, live_summary : String, live_price : Int, live_type : String, live_isText : String, path : String, keyword : List<String>){
        ...
        file.saveInBackground(object : SaveCallback(){
            override fun done(p0: AVException?) {
                if(p0 == null){
                   if(live_isText == LCConfig.LIVE_TEXT){
                       createTextLive(live_name, live_time, live_summary, live_price, live_type, keyword, file)
                   }else{
                       createVideoLive(live_name, live_time, live_summary, live_price, live_type, keyword, file)
                   }
                }else{
    ...
    private fun createVideoLive(live_name: String, live_time: Date, live_summary: String, live_price: Int, live_type: String, keyword: List<String>, file: AVFile) {
    ...
    private fun createTextLive(live_name : String, live_time : Date, live_summary : String, live_price : Int, live_type : String, keyword : List<String>, file : AVFile){
        val userId = AVUser.getCurrentUser().objectId // 用户ID
        val username = AVUser.getCurrentUser().username
